...
class InventoryattachmentController extends AbstractActionController
{
    protected $adapter;
    protected $config;
    protected $doctrine;
    ...
    public function addAction()
    {
        ...
        if (null !== $file['uploaded-file'][0] and '' !== $file['uploaded-file'][0]['name']) {
            ...
            if (in_array($extension,$this->config['authorized_file_extension'])) {
                ...
                $result = $attachment->addAttachment($file['uploaded-file'][0], $attachment::STOCK_ATTACHMENT, $id, $fileLabel);
                if ('error' === $result['result']) {
                    ...
                    $insert = $sql->insert('stock_attachment');
                    $insert->values([
    ...
    public function deleteAction(){
        ...
        $this->redirect()->toRoute('warehouse/default', ['controller' => 'inventory', 'action' => 'display', 'id' => $stockId]);
        return $this->getResponse();
    }
    ...
    public function changedefaultphotoAction(){
        ...
        $attachmentEntity = $this->doctrine->getRepository('Warehouse\Entity\StockAttachment')->findByStockId($stockId);

        foreach($attachmentEntity as $att) {
            if ($att->getId() == $id){
                $att->setDefaultPhoto(1);
                $att->setDescription("Inventory photo");
            } elseif (1 == $att->getDefaultPhoto()) {
                $att->setDefaultPhoto(0);
                $date = new \DateTime('now');
                $att->setDescription("Inventory photo - ".$date->format('Y-m-d H:i:s'));
            }
    ...
    private function resize($width, $height, $imageName, $extension, $originalimage) {
        ...
        list($w, $h) = getimagesize($this->config['path']['stock_image_upload'] . '/' .$originalimage);
        if ($h > $w) {
            $ratio = $h / $height;
            $width = ceil($w / $ratio);
        } else {
        ...
        $path      = $this->config['path']['stock_public_thumb_upload'] . '/' . $imageName;
        $fileName  = $this->config['path']['stock_image_upload'] . '/'.$originalimage;
